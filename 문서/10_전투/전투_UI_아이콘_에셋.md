...
취소는 동작 의미가 더 분명한 `arrowLeft.png`를 유지한다. `cross.png`는 투사 `난도` 스킬에만 사용하고, `skull.png`는 투사 개인 자원 `기세`와 이를 소비하는 `회심의 일격`에 사용한다. 상단 HUD는 아이콘과 `기세 n` 한글을 함께 보여 그림만으로 상태를 전달하지 않는다.
