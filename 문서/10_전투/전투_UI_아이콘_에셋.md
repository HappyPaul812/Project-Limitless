...
| 수호자 수호의 맹세 | Board Game Icons `PNG/Default (64px)/pawns.png` | `skill.guardian.cover_allies` |
| 수호의 맹세 상태 | 스킬과 같은 `pawns.png` | `status.guardian.cover_allies` |
...
수호의 맹세는 같은 팩의 실제 `pawns.png`를 버튼과 수호자 상태 HUD에 사용한다. 세 인물 실루엣으로 여러 동료 보호를 표현하고, 공용 방어 `shield.png`·가이아 웰 `dice_shield.png`·철벽 `structure_wall.png`는 변경하지 않는다. HUD는 아이콘과 `수호의 맹세` 한글을 함께 표시하고 상세 팝업에서 남은 보호 예산을 확인한다.
