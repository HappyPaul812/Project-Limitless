...
- 마음의 상처의 길: 균열을 금빛 에너지가 잇는 `이어진 심장석`
...
다음 15개 시작 스킬은 `JobDefinition` Asset의 캐릭터 생성 프리뷰 ID를 `BattleSkillCatalog`가 실제 전투 효과와 연결한다. 현재 구현 수치와 판정은 `문서/06_스킬/README.md` 및 `문서/10_전투/전투시스템.md`를 따른다.
...
직업 패시브 프리뷰와 미래 스킬 후보는 이 시작 스킬의 구현 상태와 구분하며, 미확정 수치를 임의로 확정하지 않는다.
