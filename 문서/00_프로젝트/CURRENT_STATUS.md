...
- 갱신일: 2026-08-26
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `6c6e9d0` (`Chore: 화살비 다중 후열 검증 배치`)
...
- 사수 `화살비`: `EnemyRearRowAll`의 살아 있는 적 후열 전체에 일반 공격 120% 피해, 후열 0명에서는 전열 전환 없이 행동 미소비, 자체 쿨타임 없음
...
Unity 16:9 Play Mode의 전열 A·후열 B/C 배치에서 화살비가 B/C만 거의 동시에 한 번씩 공격하고 A는 제외하는지 우선 확인한다. 이어 한 후열 전투불능 후 다음 화살비 대상 제외, 후열 0명 행동 미소비와 기존 정조준·회오리 베기·난도·회심의 일격·치유의 빛·취소 흐름 회귀를 확인한다.
