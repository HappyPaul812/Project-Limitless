...
- 마지막 전투 UI 관련 commit: `d340bac` (`Refactor: 전투 UI 선택 흐름 정리`)
...
45. 캐릭터 주변에는 대상 화살표와 `행동 중`만 보이고, 상단 HP 항목에 `행동 중`, `방어`, `도발 2/1`, 구현 스킬 `재사용 n턴`이 한 줄로 표시되는지 확인한다.
46. 캐릭터 Hover와 키보드 포커스 상세 팝업에 이름·직업/몬스터·HP 숫자·방어·도발·쿨타임이 계속 표시되고 HP HUD·명령 패널을 가리지 않는지 확인한다.
47. 메인 명령 버튼과 패널이 이전보다 약 17% 작아졌지만 글자·간격·마우스 클릭이 명확한지 확인한다.
48. 대상 선택과 스킬 메뉴에서만 `취소` 버튼이 보이고, 마우스 클릭과 Esc 모두 선택을 해제해 올바른 상위 명령 버튼으로 포커스가 복귀하는지 확인한다.
49. 공격·스킬·방어·도망, 3대3 대상 판정, 도발·방어 수치, Projectile·승패·필드 복귀가 기존과 같고 Console 오류가 없는지 확인한다.

기존 Male/Female, Path Visual과 Wheelchair Variant, 이름표, 월드 경계·전환·초원 슬라임 필드 Animation도 회귀가 없는지 함께 확인한다.

## 다음 권장 작업

Unity 16:9 Play Mode에서 Sprite 중앙 대상 화살표, HP HUD 상태 요약, 축소 명령 패널과 Esc/마우스 취소 흐름을 우선 검증한다. 이어 적 4~6명 배치와 3대3 전투 회귀를 확인한 뒤 미엘의 치유의 빛을 공용 아군 대상 선택 흐름에 연결한다.
