...
- 갱신일: 2026-08-31
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `476d63a` (`Feature: 치유사 정화 스킬 구현`)
...
- 독침벌 기본 공격은 MonsterDefinition의 120% 데이터로 슬라임 10 대비 12 피해를 사용하고 정상 적중 시 독 3 부여. 독은 대상 자신의 행동 종료마다 최대 HP 5% 올림·최소 1 피해로 `3→2→1→제거`, 재적중은 독 3 갱신이며 전투불능 시 즉시 제거
...
다음 권장 작업은 Battle Play Mode에서 정화의 단일/복합 해로운 상태 제거, 상태 없음 행동 미소비, spectral-bloom 타이밍과 치유사 행동 기준 2턴 쿨타임을 검증하는 것이다. 독침벌 12 직접 피해와 독·화상 동시 틱 회귀도 함께 확인한다.
