...
- 마지막 전투 UI 관련 commit: `c83d86a` (`Refactor: 전투 HP 목록 고정 배치`)
...
- 화면 왼쪽 적군 최대 6명·오른쪽 아군 최대 3명의 고정 HP 목록과 실제 참가자만 생성되는 이름·비율 HP 행
...
40. 16:9 Battle에서 왼쪽 적군 HP 목록 3행과 오른쪽 아군 HP 목록 3행만 표시되고, 캐릭터 주변의 이름·HP Bar가 제거되어 Sprite를 가리지 않는지 확인한다.
41. 참가자 수를 줄인 Encounter에서는 없는 참가자의 빈 HP 행이 생성되지 않고, 적 6명 구성에서는 왼쪽 목록이 최대 6행까지 표시되는지 확인한다.
42. 피해 직후 해당 고정 HP 행의 Bar 길이가 현재/최대 HP 실제 비율만큼 줄고, HP 0에서 완전히 비며 이름에 `[전투불능]`이 표시되는지 확인한다.
43. 방어 중 참가자에 `방어`, 도발된 슬라임에 `도발 2/1`, 현재 행동자와 선택 대상 표식이 캐릭터 가까이에 계속 보이는지 확인한다.
44. 캐릭터 Hover와 대상 선택 중 키보드 포커스 모두 같은 단일 상세 팝업을 열고 이름·직업/몬스터·HP 숫자·방어·도발·쿨타임 중 현재 값만 표시하는지 확인한다.
45. 캐릭터 Hover 또는 키보드 대상 포커스 이동 시 해당 캐릭터의 HP 목록 행도 밝은 배경·금색 테두리로 함께 강조되고, Esc 취소 후 정상 해제되는지 확인한다.
46. 좌우 HP 목록과 상세 팝업이 캐릭터 Sprite·타임라인·하단 공격/스킬/방어/도망 패널을 가리지 않으며 하단 패널 크기와 위치가 기존과 같은지 확인한다.
47. 타임라인, 3대3 조작, 피해·방어·도발·Projectile·승패·도망·필드 복귀가 기존과 같고 Console에 Compile Error, NullReferenceException, MissingReferenceException이 없는지 확인한다.

기존 Male/Female, Path Visual과 Wheelchair Variant, 이름표, 월드 경계·전환·초원 슬라임 필드 Animation도 회귀가 없는지 함께 확인한다.

## 다음 권장 작업

Unity 16:9 Play Mode에서 좌우 고정 HP 목록의 배치·실제 비율 감소·포커스 행 연동과 상세 팝업을 우선 검증한다. 이어 3대3 전투 회귀를 확인한 뒤 미엘의 치유의 빛을 공용 아군 대상 선택 흐름에 연결한다.
