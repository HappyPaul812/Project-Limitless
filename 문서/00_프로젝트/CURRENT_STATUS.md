...
- 갱신일: 2026-08-28
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `d1ede85` (`Feature: 마도사 가이아 웰 구현`)
...
- 마도사 `가이아 웰`: 자기 자신에게 받는 피해 40% 감소를 부여하고 현재 행동을 제외한 자신의 다음 2회 행동 종료에서만 감소, 마도사 행동 기준 4턴 쿨타임 구현
- 가이아 웰은 원시 피해를 60%로 만든 뒤 기존 `TakeDamage()`의 방어 50%를 유지해 동시 활성 시 순차 곱연산(최종 30%)으로 처리. 도발·다른 아군 보호·대상 선택 없음
...
- **마도사 가이아 웰 구현**: 자신 전용 40% 받는 피해 감소, 사용 행동 제외 다음 자기 행동 2회 지속, 자기 행동 기준 4턴 쿨타임을 기존 상태·쿨타임 구조에 연결했다. 기존 방어 코드를 변경하지 않는 순차 곱연산과 arcane-parry 16프레임 peak 적용, `dice_shield.png` HUD를 추가했고 전체 `Assembly-CSharp` 참조 컴파일 오류 0개(기존 deprecated API 경고 4개)를 확인했다. 실제 VFX 위치·피해 반올림·2회 수명은 Play Mode 확인이 필요하다.
...
129. 마도사 스킬 메뉴의 가이아 웰에 `dice_shield.png` 아이콘과 자기 보호·자신·받는 피해 40% 감소·자신의 다음 2회 행동·재사용 4턴 설명이 정확히 표시되는지 확인한다.
...
133. 가이아 웰 활성 중 원시 피해 10이 6으로 줄어드는지 확인하고, 일반 방어도 함께 활성인 경우 기존 방어가 뒤이어 적용되어 3 피해가 되는지 확인한다. 최소 피해 1과 올림 정책도 유지되어야 한다.
...
다음 권장 작업은 Play Mode에서 마도사 `파이어 볼`·`썬더볼트`·`가이아 웰`의 VFX peak, 상태 수명, 피해 감소 조합과 쿨타임을 통합 검증하는 것이다. 이후 1차 직업별 시작 스킬의 밸런스·회귀 검증을 진행하고, 야수 선택·장착·저장 UI 및 Wolf/Bear/Fox 패시브는 별도 기획 확정 뒤 `BeastCompanionCatalog` 경계에 연결한다.
