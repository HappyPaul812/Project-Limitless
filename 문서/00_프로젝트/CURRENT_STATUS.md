...
- 갱신일: 2026-08-26
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `0a365f8` (`Feature: 난도 근거리 공격 구현`)
...
- 투사 난도: 기존 근거리 TargetResolver로 적 1명을 선택하고 전진 타격 순간 일반 공격 150% 피해와 자신 난도 +1 적용
- 기존 근거리 기본 공격 Presenter를 재사용하는 전진→타격·피해 숫자·피격 반응→원위치 복귀→다음 턴 흐름
- 참가자별 0~3중첩 자원, 세 번째 직접 사용으로 3중첩이 될 때만 투사 행동 기준 2턴 쿨타임
- 난도 자원 증가·직접 사용 기록·전부 소비 API 분리로 회오리 베기 명중당 획득과 회심의 일격 100/130/160/190%·전부 소비 확장 준비
- 투사 스킬 버튼과 상단 HP HUD에 Kenney `cross` 아이콘·`난도 n`, 상세 상태 팝업에 `난도 0/3~3/3` 표시, 전투불능 시 숨김
...
- 투사 난도를 제외한 회심의 일격·회오리 베기 등 나머지 직업별 스킬 효과와 길 패시브
...
- 투사 난도의 근거리 대상 선택·150% 타격·적중 후 자원 증가·직접 사용 3회 쿨타임·HUD 표시를 Unity 6000.5.7f1 전체 `Assembly-CSharp` 참조로 컴파일해 오류 0개를 확인했다. 공격력 12→18, 15→23, 20→30의 정수 올림 계산을 확인했고 기존 deprecated API 경고 4개만 있었다. Formation·TurnOrderQueue·BattleCore·Projectile/VFX와 기존 세 직업 스킬은 변경하지 않았다. 실제 타격 시점과 UI 배치는 Play Mode 확인이 필요하다.
...
71. 플레이어 직업을 투사로 선택해 스킬 메뉴에 cross 아이콘과 `난도`만 기본 표시되고 Hover·키보드 포커스 설명에 적 1명, 근거리 물리, 일반 공격 150%, 적중 후 난도 +1, 최대 3, 세 번째 사용 후 2턴이 표시되는지 확인한다.
72. 난도 선택 시 기존 근거리 TargetResolver와 같은 대상만 금색으로 활성화되고, 대상 선택 중 Esc/취소는 스킬 메뉴로 돌아가는지 확인한다.
73. 난도 사용 시 투사가 전진하고 실제 타격 순간에만 150% 피해·피해 숫자·피격 반응과 난도 +1이 적용된 뒤 원위치로 복귀하고 다음 턴으로 진행하는지 확인한다.
74. 공격력 12인 투사의 방어하지 않는 대상 피해가 18이며, 방어 중 대상에는 기존 50% 감소가 적용되는지 확인한다.
75. 상단 투사 HUD가 `난도 1 → 2 → 3`, 상세 팝업이 전투 시작부터 `난도 0/3 → 1/3 → 2/3 → 3/3`으로 갱신되는지 확인한다.
76. 세 번째 난도 직접 사용 직후 `난도 3`과 `난도 재사용 2턴`이 함께 표시되고, 투사의 다음 행동 시작에 1턴, 그다음 행동 시작에 사용 가능으로 감소하는지 확인한다.
77. 난도 3중첩에서 다시 선택하면 `난도가 이미 최대입니다.` 안내 후 스킬 메뉴에 남고 행동이 소비되지 않는지 확인한다.
78. 투사가 전투불능이 되면 난도·쿨타임 아이콘과 글자가 숨겨지고 회색 `전투불능`만 남는지 확인한다.
79. 수호자 도발·치유사 치유의 빛·사수 정조준과 기본 공격·방어·취소·승패·도망·Field 복귀가 기존과 같고 Console 오류가 없는지 확인한다.

기존 Male/Female, Path Visual과 Wheelchair Variant, 이름표, 월드 경계·전환·초원 슬라임 필드 Animation도 회귀가 없는지 함께 확인한다.

## 다음 권장 작업

Unity 16:9 Play Mode에서 투사 난도의 근거리 대상 판정, 전진 타격 순간 150% 피해·난도 증가, 0/3→3/3 UI, 세 번째 직접 사용 후 2→1→사용 가능 흐름과 최대 상태 행동 미소비를 우선 확인한다. 이어 기존 도발·치유의 빛·정조준 회귀를 확인하고 회심의 일격 또는 회오리 베기 실제 구현을 선정한다.
