...
- 마지막 전투 UI 관련 commit: `c79a8fe` (`Refactor: 전투 상태 UI 간소화`)
...
- 캐릭터 근처 이름·미니 HP Bar 상시 표시와 Hover/마우스 선택/키보드 포커스 공용 단일 상세 상태 팝업
...
40. 16:9 Battle에서 기존 6개 고정 대형 상태창이 사라지고 각 캐릭터 가까이에 이름과 작은 HP Bar만 표시되는지 확인한다.
41. 방어 중 참가자에 `방어`, 도발된 슬라임에 `도발 2/1` 표식이 팝업 없이도 보이는지 확인한다.
42. 캐릭터 Hover와 대상 선택 중 키보드 포커스 모두 같은 단일 상세 팝업을 열고 이름·직업/몬스터·HP·방어·도발·쿨타임 중 현재 값만 표시하는지 확인한다.
43. 화면 좌우 끝 참가자의 팝업이 안쪽에 배치되고 화면 밖이나 다른 미니 HUD를 심하게 가리지 않는지 확인한다.
44. 공격 대상 포커스 이동 시 강조·화살표·팝업 내용이 즉시 함께 이동하고 Esc 취소 후 명령 메뉴로 복귀하는지 확인한다.
45. 타임라인, 3대3 조작, 피해·방어·도발·Projectile·승패·도망·필드 복귀가 기존과 같은지 확인한다.
46. Console에 Compile Error, NullReferenceException, MissingReferenceException이 없는지 확인한다.

기존 Male/Female, Path Visual과 Wheelchair Variant, 이름표, 월드 경계·전환·초원 슬라임 필드 Animation도 회귀가 없는지 함께 확인한다.

## 다음 권장 작업

Unity 16:9 Play Mode에서 미니 HP 표시, Hover·키보드 포커스 상세 팝업, 가장자리 자동 배치와 대상 선택 연동을 우선 검증한다. 이어 3대3 전투 회귀를 확인한 뒤 미엘의 치유의 빛을 공용 아군 대상 선택 흐름에 연결한다.
