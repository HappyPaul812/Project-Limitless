...
- 갱신일: 2026-08-26
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `b3c5434` (`Fix: 회오리 베기 전열 범위 적용`)
...
- 스킬 전용 광역 범위 `EnemyFrontRowAll`·`EnemyRearRowAll`·`EnemyAll`과 공용 해석기 구조. 회오리 베기는 전열, 향후 화살비는 후열, 향후 썬더볼트는 적 전체 범위로 확정
...
Unity 16:9 Play Mode에서 회오리 베기의 전열 1~3명 80% 동시 피해·후열 완전 제외·적중당 기세 획득과 전열 0명 행동 미소비를 우선 확인한다. 이어 난도 쿨타임 비발생, 회심의 일격 190% 소비 연계와 기존 도발·치유의 빛·정조준·취소 흐름 회귀를 확인한다.
