...
- 갱신일: 2026-08-24
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `617d716` (`Feature: 기본 공격 액션 연출 1단계 추가`)
...
- `BattleCore.cs`, `TargetResolver`, `Formation`, 피해·방어 계산은 변경하지 않았고 원거리·마법 기본 공격은 기존 즉시 처리 흐름을 유지했다. 실제 이동 간격·피격 점멸·피해 숫자·입력 잠금은 Play Mode 확인이 필요하다.
...
Unity가 새 스크립트를 import한 뒤 근거리 플레이어와 초원 슬라임의 기본 공격 전진·피격·피해 숫자·입력 잠금을 우선 검증한다. 이어 양측 HP Bar, 승리·도망별 스폰 유지와 30초 독립 리스폰 회귀를 확인한다. 원거리·마법 기본 공격 연출, 직업별 스킬, Projectile, 카메라 흔들림과 사운드는 후속 작업으로 남긴다.
