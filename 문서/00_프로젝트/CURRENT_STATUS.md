...
- 갱신일: 2026-09-02
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `08ed041` (`Fix: 아군 VFX 색상과 주사위 연출 수정`)
...
- 행동 우선도→민첩→전투 시작 1회 Tie Break 순서 구현. 실제 Combatant 참조별 값을 전투 동안 유지하고, 민첩 동률 전투에서만 Overlay를 열어 기존 계산과 분리된 1~6 눈을 0.09초 간격으로 1.2초간 바꾼 뒤 첫 행동 결과를 표시. Kenney `dice_shield.png`는 실제 눈 Sprite가 아닌 방패라 기존 UI Image·점 문자를 조합하며 입력 잠금과 실제 첫 행동/Timeline 판정 유지
- 회복의 파동·정화·철벽·대신 막기·가이아 웰은 기존 Sprite와 적용 프레임을 유지하고 공용 Presenter의 Image 생성 시 스킬 Tint를 함께 확정해 흰 단색 실루엣을 방지. Tint 미지정 적 대상 VFX는 Color.white multiplier로 원본색 유지
...
181. 회복의 파동·정화·철벽·대신 막기·가이아 웰이 흰 단색이 아니라 각 원본/의도 색조로 보이고 기존 peak/release 시점에 효과가 적용되는지 확인한다.
182. 파이어 볼·썬더볼트·화살비·동료의 습격·독/화상/감전·숲거미 독액 분사의 기존 색과 피해 횟수가 유지되는지 확인한다.
183. 민첩 동률 전투에서 Overlay의 주사위가 회전하지 않고 1~6 눈을 빠르게 바꾼 뒤 멈추며, 결과 문구·Timeline·첫 행동자가 일치하고 다음 라운드에는 재표시되지 않는지 확인한다.
184. 민첩 동률이 없는 전투에서는 Overlay가 없고 바로 정상 입력으로 시작하는지 확인한다.

관련 코드·전투 문서는 `git diff --check`를 통과했고 Unity 6000.5.7f1 배치 실행은 종료 코드 0으로 완료됐다. 실제 색상 합성, 주사위 눈 가독성 및 위 181~184 항목은 Play Mode에서 직접 확인해야 한다.
