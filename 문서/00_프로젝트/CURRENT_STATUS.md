...
- 주민 대표 `starter-village-main-guide`, 남문 경비병 `starter-village-gate-guard`, 태온 `companion_taeon`, 미엘 `companion_miel`의 Resources 정의를 준비했다. 태온에는 공식 초상화 `Portrait_Taeon.png` 적용을 완료했으며, 나머지 세 정의는 Sprite를 연결하기 전까지 기존 null-safe 글자 배치를 유지한다. 콘텐츠 생성 메뉴는 기존 연결 Sprite를 보존한다. 마지막 기능 commit: `47f2318`.
