...
- 갱신일: 2026-08-25
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `560f1b8` (`Feature: 투사 난도 전투 자원 구현`)
...
- 투사 난도: 대상 선택 없는 자기 준비 행동, 참가자별 0~3중첩 자원, 세 번째 직접 사용 후 투사 행동 기준 2턴 쿨타임
- 난도 자원 증가·직접 사용 기록·전부 소비 API 분리로 회오리 베기 명중당 획득과 회심의 일격 100/130/160/190%·전부 소비 확장 준비
- 투사 스킬 버튼과 상단 HP HUD·상세 상태 팝업에 Kenney `cross` 아이콘과 `난도 n` 표시, 전투불능 시 숨김
...
- 투사 난도 데이터·참가자별 자원·직접 사용 3회 쿨타임·HUD 표시 연결을 Unity 6000.5.7f1 전체 `Assembly-CSharp` 참조로 컴파일해 오류 0개를 확인했다. 기존 deprecated API 경고 4개만 있었고, TargetResolver·Formation·TurnOrderQueue·BattleCore·Projectile/VFX와 기존 세 직업 스킬은 변경하지 않았다. 실제 0→1→2→3, 2턴 쿨타임, 최대 상태 행동 미소비와 UI 배치는 Play Mode 확인이 필요하다.
...
71. 플레이어 직업을 투사로 선택해 스킬 메뉴에 cross 아이콘과 `난도`만 기본 표시되고 Hover·키보드 포커스 설명에 대상 자신, 난도 +1, 최대 3, 세 번째 사용 후 2턴이 표시되는지 확인한다.
72. 난도를 사용할 때 대상 선택 없이 `난도!` 강조 후 행동이 소비되고, 적 HP는 변하지 않으며 상단 투사 HUD와 상세 팝업이 `난도 1 → 2 → 3`으로 갱신되는지 확인한다.
73. 세 번째 난도 직접 사용 직후 `난도 3`과 `난도 재사용 2턴`이 함께 표시되고, 투사의 다음 행동 시작에 1턴, 그다음 행동 시작에 사용 가능으로 감소하는지 확인한다.
74. 난도 3중첩에서 다시 선택하면 `난도가 이미 최대입니다.` 안내 후 스킬 메뉴에 남고 행동이 소비되지 않는지 확인한다.
75. 투사가 전투불능이 되면 난도·쿨타임 아이콘과 글자가 숨겨지고 회색 `전투불능`만 남는지 확인한다.
76. 수호자 도발·치유사 치유의 빛·사수 정조준과 기본 공격·방어·취소·승패·도망·Field 복귀가 기존과 같고 Console 오류가 없는지 확인한다.

기존 Male/Female, Path Visual과 Wheelchair Variant, 이름표, 월드 경계·전환·초원 슬라임 필드 Animation도 회귀가 없는지 함께 확인한다.

## 다음 권장 작업

Unity 16:9 Play Mode에서 투사 난도의 대상 없는 즉시 사용, 0→3 HUD, 세 번째 직접 사용 후 2→1→사용 가능 흐름과 최대 상태 행동 미소비를 우선 확인한다. 이어 기존 도발·치유의 빛·정조준 회귀를 확인하고 회심의 일격 또는 회오리 베기 실제 구현을 선정한다.
