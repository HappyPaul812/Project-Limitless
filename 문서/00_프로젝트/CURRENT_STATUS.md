...
- 갱신일: 2026-08-31
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `9e2e655` (`Feature: 독침벌 독 상태 구현`)
...
다음 권장 작업은 Battle Play Mode에서 독침벌 12 직접 피해, 독 3→2→1 틱·재적용·전투불능 제거와 화상 동시 처리까지 검증하는 것이다. 이후 치유사 `정화`가 `BattleStatusEffectRuntime`의 독을 제거하도록 연결한다.
