...
- 마지막 전투 UI 관련 commit: `742e64d` (`Refactor: 전투불능 UI 상태 정리`)
...
- 별도 외부 자산 없이 단색 기호+텍스트를 함께 사용하는 공격·스킬·방어·도망·취소 버튼과 HP HUD 상태 아이콘
...
45. 캐릭터 주변에는 대상 화살표와 `행동 중`만 보이고, 상단 HP 항목에 단색 기호와 함께 `행동 중`, `방어`, `도발 2/1`, 구현 스킬 `재사용 n턴`이 한 줄로 표시되는지 확인한다.
...
49. 공격·스킬·방어·도망·취소 버튼에 작은 단색 기호와 텍스트가 함께 보이며 글자가 깨지거나 버튼 크기가 커지지 않았는지 확인한다.
...
Unity 16:9 Play Mode에서 전투불능 직후 상태 정리, 회색 발판 제거, 단색 버튼/상태 기호와 죽은 대상 포커스 정리를 우선 검증한다. 이어 적 4~6명 배치와 3대3 전투 회귀를 확인한 뒤 미엘의 치유의 빛을 공용 아군 대상 선택 흐름에 연결한다.
