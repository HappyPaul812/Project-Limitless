...
- 갱신일: 2026-08-28
- 기준 브랜치: `main`
- 마지막 기능 관련 commit: `3f4d76f` (`Feature: 마도사 파이어 볼 구현`)
...
다음 권장 작업은 Play Mode에서 마도사 `파이어 볼`의 Charge·큰 Projectile·폭발 peak 피해·행동 종료 화상 흐름을 검증한 뒤 마도사 `썬더볼트` 규칙을 확정하는 것이다. 야수 쪽은 Wolf 접촉 위치·속도·크기를 함께 확인하고, 별도 기획이 확정되면 선택·장착·저장 UI와 Wolf/Bear/Fox 패시브를 `BeastCompanionCatalog` 경계에 연결한다.
