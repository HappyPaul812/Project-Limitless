...
- 실시간 탐험과 턴제 전투의 구분
- 싱글플레이 행동 선택은 시간 제한 없음
- 멀티플레이는 동시 입력, 기본 45초 제한, 남은 시간 10초 경고, 시간초과 시 방어
- 복수 대상 스킬은 고정된 `적 전체` 또는 `주변 모든 적` 표현 대신 공통 Target Range 판정을 따름
- AP, 상태이상, 행동·협동 기술, 보스 패턴은 미확정이며 구현하지 않음
- 위 항목은 문서 설계만 완료했으며 Battle Scene과 C# 전투 로직은 구현하지 않음
...
- 전투·직업·접근성 문서의 입력 시간과 Target Range 표현을 정적으로 교차 확인했다.
- 이번 전투 규칙 정리는 문서만 변경했으며 Unity Editor 또는 Play Mode 검증 대상이 아니다.
...
Target Range의 구체적인 데이터 표현과 배치 판정, AP·상태이상·행동 및 협동 기술·보스 패턴의 채택 여부를 각각 확정한 뒤 최소 턴제 전투 데이터 경계를 설계한다. Scene 전환 여부, 조우 후 필드 복귀, 반복 조우와 몬스터 제거 규칙도 구현 전에 사용자에게 선택지를 확인한다.
